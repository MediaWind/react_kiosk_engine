...
export default function useAppointment(dispatchAppointment: React.Dispatch<IAppointmentAction>, dispatchError: React.Dispatch<IErrorAction>, dispatchAppointments: React.Dispatch<IAppointmentsAction>): [string, CallableFunction, CallableFunction, CallableFunction] {
	...
	async function checkOut(qrCode: string) {
		...
		try {
			...
			if (data.status != 1) {
				Console.error("Error when trying to check out appointment: data status " + data.status, { fileName: "useAppointment", functionName: "checkOut", lineNumber: 139, });
	...
	async function getAppointments(birthDate: string | null = null, nationalNumber: string | null = null, minBeforeAppointment = null, minAfterAppointment = null, services: Array<number> | null = null) : Promise<object | undefined> {
		...
		try {
			...
			if (data.status === 1) {
				...
				Console.error("Error when trying to get appointments: data status " + data.status, { fileName: "useAppointment", functionName: "getAppointments", lineNumber: 247, });
				if (data.status_msg && data.status_msg === "appointment_not_found") {
